...
def get_soft_new(t_vector, score, eps=0.25):
    eps = eps * (1 - score)
    max_val = 1 - eps
    min_val = eps / (t_vector.shape[-1] - 1)

    min_val = min_val.unsqueeze(1).expand_as(t_vector)
    max_val = max_val.unsqueeze(1).expand_as(t_vector)

    t_soft = torch.empty(t_vector.shape).cuda()
    t_soft[t_vector == 0] = min_val[t_vector == 0]
    t_soft[t_vector == 1] = max_val[t_vector == 1]
...
class SoftDICELoss(nn.Module):
    ...
    def forward(self, output, target, return_class=False, score=None, loss_method_num=0):
        input_device = output.device
        # temporal solution to avoid nan
        output = output
        target = target
        ...
        if score is not None and (loss_method_num==1 or loss_method_num==3):
            target_soft = get_soft_new(target_onehot, score, eps=self.eps).cuda()
        else:
            target_soft = get_soft(target_onehot, eps=self.eps).cuda()

        # target_soft = get_soft(target_onehot, eps=self.eps).cuda()

        output = F.softmax(output, dim=-1)
        
        if score is not None and (loss_method_num==2 or loss_method_num==3):
            intersection = (output * target_soft * score.unsqueeze(1).expand_as(output)).sum(dim=0)
        else:
